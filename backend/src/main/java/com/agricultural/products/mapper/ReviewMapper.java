...
public interface ReviewMapper {
    ...
    @Select("SELECT COUNT(*) FROM review WHERE user_id = #{userId} AND product_id = #{productId} AND parent_id IS NULL")
    int countUserProductReviews(@Param("userId") Long userId, @Param("productId") Long productId);

    @Select("SELECT COUNT(*) FROM review WHERE user_id = #{userId} AND product_id = #{productId} AND order_id = #{orderId} AND parent_id IS NULL")
    int countUserOrderProductReviews(@Param("userId") Long userId, @Param("productId") Long productId, @Param("orderId") Long orderId);
    ...
    @Select("""
            ...
            LIMIT 1
            """)
    Long findFirstReviewableOrderId(@Param("userId") Long userId, @Param("productId") Long productId);

    @Select("SELECT COUNT(*) FROM `order` o JOIN order_item oi ON o.id = oi.order_id WHERE o.user_id = #{userId} AND oi.product_id = #{productId} AND o.status = 3")
    int countCompletedPurchase(@Param("userId") Long userId, @Param("productId") Long productId);

    @Select("SELECT COUNT(*) FROM `order` o JOIN order_item oi ON o.id = oi.order_id WHERE o.id = #{orderId} AND o.user_id = #{userId} AND oi.product_id = #{productId} AND o.status = 3")
    int countCompletedOrderItem(@Param("userId") Long userId, @Param("productId") Long productId, @Param("orderId") Long orderId);
