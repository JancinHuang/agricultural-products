...
function isPresignedUrl(url) {
  return /^https?:\/\//i.test(url)
    && (url.includes('X-Amz-') || url.includes('Signature=') || url.includes('AWSAccessKeyId=') || url.includes('zos.ctyun.cn'))
}
...
export const imageUtils = {
  ...
  getImageUrl(url, options = {}) {
    if (!url) return IMAGE_CONFIG.placeholder
    const baseUrl = resolveImageUrl(url)
  ...
  loadImageWithRetry(url, img, retries = IMAGE_CONFIG.retryCount) {
    ...
    img.onerror = (event) => {
      if (retries > 0) {
        ...
        setTimeout(() => {
          // 添加时间戳避免缓存
          const retryUrl = isPresignedUrl(url)
            ? url
